...
def ny_to_utc_dt(t_local_ny: datetime) -> datetime:
    """Convert NY local naive datetime to UTC naive datetime (fixed -4h offset)."""
    return t_local_ny - timedelta(hours=NY_UTC_OFFSET_HOURS)
...
def draw_candles(ax, candles: List[Candle], title: str, *,
                 ...
                 session_markers: Optional[List[Tuple[float, str, str]]] = None):
    ax.clear()
    ...
    # --- draw session markers (vertical dashed lines + labels) ---
    if session_markers:
        ymin, ymax = ax.get_ylim()
        ylab = ymax - (ymax - ymin) * 0.02  # near top
        for x, label, color in session_markers:
            ax.axvline(x, color=color, linestyle="--", linewidth=1.1, alpha=0.9, zorder=1)
            ax.text(x + 0.05, ylab, label, color=color, fontsize=9, rotation=90,
                    va="top", ha="left", alpha=0.95, fontweight="bold", zorder=4)
...
class App(tk.Tk):
    ...
    def _apply_styles(self):
        ...
        # Themed dropdown listbox + selections
        try:
    ...
    def _update_all_cards(self):
        ...
        if hasattr(self, "_left_canvas"):
            try: self._left_canvas.configure(bg=self.palette["CARD_BG"])
            except Exception: pass
    ...
    def _build_layout(self):
        ...
        self._left_canvas = tk.Canvas(left_card.inner, bg=self.palette["CARD_BG"], highlightthickness=0)
        self._left_vsb = ttk.Scrollbar(left_card.inner, orient="vertical", command=self._left_canvas.yview)
        self._left_canvas.configure(yscrollcommand=self._left_vsb.set)
        self._left_canvas.pack(side=tk.LEFT, fill=tk.Y, expand=True)
        self._left_vsb.pack(side=tk.RIGHT, fill=tk.Y)

        left = ttk.Frame(self._left_canvas)
        self._left_window = self._left_canvas.create_window((0,0), window=left, anchor="nw")

        left.bind("<Configure>", lambda e: self._left_canvas.configure(scrollregion=self._left_canvas.bbox("all")))
        self._left_canvas.bind("<Configure>", lambda e: self._left_canvas.itemconfig(self._left_window, width=e.width))

        # Mouse-wheel scroll (left panel only)
        ...
        def _bind_scroll(_): self._left_canvas.bind_all("<MouseWheel>", _on_mousewheel)
        def _unbind_scroll(_): self._left_canvas.unbind_all("<MouseWheel>")
        self._left_canvas.bind("<Enter>", _bind_scroll)
        self._left_canvas.bind("<Leave>", _unbind_scroll)
        self._left_canvas.bind_all("<Button-4>", lambda e: self._left_canvas.yview_scroll(-1, "units"))
        self._left_canvas.bind_all("<Button-5>", lambda e: self._left_canvas.yview_scroll(1, "units"))
    ...
    def _refresh_advanced_enabled(self):
        ...
        # Redraw so markers appear/disappear instantly
        self._redraw_current()
    ...
    # ---------- Session markers ----------
    def _compute_session_markers(self, candles: List[Candle]) -> List[Tuple[float, str, str]]:
        """Return list of (x_position, label, color) for LDN/NY open/close boundaries."""
        markers: List[Tuple[float, str, str]] = []
        if not candles or not self.var_adv_on.get():
            return markers

        show_ldn = self.var_adv_london.get()
        show_ny  = self.var_adv_ny.get()

        # Colors chosen to be visible in both themes
        col_ldn = "#F59E0B"   # amber
        col_ny  = "#60A5FA"   # light blue

        # Iterate consecutive candle times in UTC and test crossings
        times_ny = [c.t for c in candles]
        times_utc = [ny_to_utc_dt(t) for t in times_ny]
        ...
        for i in range(1, len(times_utc)):
            ...
            # Check boundaries for both dates that span this segment
            dates_to_check = {t0.date(), t1.date()}
            for d in list(dates_to_check):
                # London session boundaries
                if show_ldn:
                    ldn_open = datetime.combine(d, dtime(8, 0))
                    ldn_close = datetime.combine(d, dtime(17, 0))
                    if t0 < ldn_open <= t1:
                        frac = (ldn_open - t0).total_seconds() / (t1 - t0).total_seconds()
                        markers.append(( (i-1) + frac, "LDN open", col_ldn ))
                    if t0 < ldn_close <= t1:
                        frac = (ldn_close - t0).total_seconds() / (t1 - t0).total_seconds()
                        markers.append(( (i-1) + frac, "LDN close", col_ldn ))

                # New York session boundaries
                if show_ny:
                    ny_open = datetime.combine(d, dtime(13, 30))
                    ny_close = datetime.combine(d, dtime(20, 0))
                    if t0 < ny_open <= t1:
                        frac = (ny_open - t0).total_seconds() / (t1 - t0).total_seconds()
                        markers.append(( (i-1) + frac, "NY open", col_ny ))
                    if t0 < ny_close <= t1:
                        frac = (ny_close - t0).total_seconds() / (t1 - t0).total_seconds()
                        markers.append(( (i-1) + frac, "NY close", col_ny ))

        # Sort by x for consistent layering
    ...
    def _redraw_current(self):
        ...
        session_markers = self._compute_session_markers(candles)
        draw_candles(
            self.ax,
            candles,
            title,
    ...
    def on_generate(self, *_):
        try:
            ...
            draw_candles(self.ax, candles, title,
                         ...
                         chart_bg=self.var_chart_bg.get(), show_grid=self.var_show_grid.get(),
                         pal=self.palette, session_markers=session_markers)
    ...
    def _connect_zoom_controls(self):
        """Bind scroll-wheel zoom (anchored to mouse), double-click reset, and +/- keys."""
        self._zoom_base = 1.15  # zoom speed factor
        self.canvas.mpl_connect("scroll_event", self._on_mpl_scroll)
        self.canvas.mpl_connect("button_press_event", self._on_mpl_doubleclick)

        w = self.canvas.get_tk_widget()
        w.bind("<Enter>", lambda e: w.focus_set())
        w.bind("<KeyPress-plus>", lambda e: self._zoom_keyboard(True))
        w.bind("<KeyPress-equal>", lambda e: self._zoom_keyboard(True))  # '=' without shift
        w.bind("<KeyPress-minus>", lambda e: self._zoom_keyboard(False))

    def _autoscale_view(self):
        cds = self.last_candles
        if not cds: return
        lows, highs = [c.l for c in cds], [c.h for c in cds]
        pmin, pmax = (min(lows), max(highs)) if lows else (0, 1)
        pad = (pmax - pmin) * 0.04 if pmax > pmin else 1.0
        self.ax.set_ylim(pmin - pad, pmax + pad)
        self.ax.set_xlim(-1, len(cds))
        self.canvas.draw_idle()

    def _on_mpl_doubleclick(self, event):
        if event.inaxes is self.ax and getattr(event, "dblclick", False):
            self._autoscale_view()
            # redraw markers after autoscale (labels position depends on y-lims)
            self._redraw_current()

    def _zoom_keyboard(self, zoom_in: bool):
        ax = self.ax
        x0, x1 = ax.get_xlim(); y0, y1 = ax.get_ylim()
        cx, cy = (x0 + x1) / 2.0, (y0 + y1) / 2.0
        scale = 1.0 / self._zoom_base if zoom_in else self._zoom_base
        self._apply_zoom(ax, cx, cy, scale)

    def _on_mpl_scroll(self, event):
        if event.inaxes is not self.ax:
            return
        if event.button == "up":
            scale = 1.0 / self._zoom_base  # zoom in
        elif event.button == "down":
            scale = self._zoom_base        # zoom out
        else:
            return
        x = event.xdata if event.xdata is not None else (self.ax.get_xlim()[0] + self.ax.get_xlim()[1]) / 2.0
        y = event.ydata if event.ydata is not None else (self.ax.get_ylim()[0] + self.ax.get_ylim()[1]) / 2.0
        self._apply_zoom(self.ax, x, y, scale)

    def _apply_zoom(self, ax, x_anchor, y_anchor, scale):
        x0, x1 = ax.get_xlim(); y0, y1 = ax.get_ylim()

        new_x0 = x_anchor - (x_anchor - x0) * scale
        new_x1 = x_anchor + (x1 - x_anchor) * scale
        new_y0 = y_anchor - (y_anchor - y0) * scale
        new_y1 = y_anchor + (y1 - y_anchor) * scale

        if self.last_candles:
            xmin_cap, xmax_cap = -1.0, float(len(self.last_candles))
            span_min_x = 0.5
            if new_x1 - new_x0 < span_min_x:
                mid = (new_x0 + new_x1) / 2.0
                new_x0, new_x1 = mid - span_min_x/2.0, mid + span_min_x/2.0
            new_x0 = max(xmin_cap, new_x0); new_x1 = min(xmax_cap, new_x1)

        span_min_y = 1e-6
        if new_y1 - new_y0 < span_min_y:
            mid = (new_y0 + new_y1) / 2.0
            new_y0, new_y1 = mid - span_min_y/2.0, mid + span_min_y/2.0
